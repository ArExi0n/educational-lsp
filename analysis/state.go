	...
	for row, line := range strings.Split(text, "\n") {
		if strings.Contains(line, "VS Code") {
			...
			diagnostics = append(diagnostics, lsp.Diagnostic{
				...
				Severity: 1,
				Source:   "Common Sense",
				Message:  "Out of 100s for text editor you alone are the disgraced one",
			})
		}
		...
		if strings.Contains(line, "Neovim") {
			...
			diagnostics = append(diagnostics, lsp.Diagnostic{
				...
				Severity: 2,
				Source:   "Common Sense",
				Message:  "Through out heaven and earth you alone are the honered one",
			})
...
func (s *State) TextDocumentCompletion(id int, uri string) lsp.CompleteResponse {
